...
class TrajectoryDataset(Dataset):
    def __init__(self, train, config):
        ...
        self.features = torch.from_numpy(np.load(config.traj_trainset_npy if train else config.traj_testset_npy))
        self.shape = self.features.shape
    ...
    def statistics(self, dim=(0, 1)):
        # load and return mean and std if they exist
        mean_path = os.path.join(self.config.dataset_dir, f"traj_mean_{dim}.pt")
        std_path  = os.path.join(self.config.dataset_dir, f"traj_std_{dim}.pt")
