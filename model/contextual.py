...
from .transformer import MultiHeadContextualBiasedAttention, ModifiedPoswiseFeedForwardNet
from .gan import PatchDiscriminator
from pymovis.learning.transformer import MultiHeadBiasedAttention, PoswiseFeedForwardNet
...
class ContextualTransformer(nn.Module):
    def __init__(self, d_motion, config, is_context=False):
        ...
        for _ in range(self.n_layers):
            self.attn_layers.append(MultiHeadContextualBiasedAttention(self.d_model, self.d_head, self.n_heads, dropout=self.dropout) if self.is_context else MultiHeadBiasedAttention(self.d_model, self.d_head, self.n_heads, dropout=self.dropout, pre_layernorm=self.pre_layernorm))
            self.pffn_layers.append(ModifiedPoswiseFeedForwardNet(self.d_model, self.d_ff, dropout=self.dropout) if self.is_context else PoswiseFeedForwardNet(self.d_model, self.d_ff, dropout=self.dropout, pre_layernorm=self.pre_layernorm))
    ...
    def get_atten_mask(self, constrained_frames, dist_threshold):
        ...
        constrained_frames = ~torch.all(attn_mask, dim=1)

        return attn_mask, constrained_frames
    ...
    def forward(self, motion, traj, mask=None):
        ...
        if self.is_context:
            ...
            kernel_size = math.ceil(num_transition_frames / 2 / (self.n_layers - 1)) * 2 + 1
            for i in range(self.n_layers):
                atten_mask, constrained_frames = self.get_atten_mask(constrained_frames, kernel_size // 2)
                attn = self.attn_layers[i](x, x, atten_bias, mask=atten_mask)
                pffn = self.pffn_layers[i](torch.cat([x, attn], dim=-1))
                x = x + attn + pffn
        else:
            for i in range(self.n_layers):
...
class ContextualGAN(nn.Module):
    def __init__(self, d_motion, config, is_context=False):
        super(ContextualGAN, self).__init__()

        self.G = ContextualGenerator(d_motion, config, is_context)
        self.D = PatchDiscriminator(d_motion, config)
    
    def generate(self, motion, traj, mask=None):
        recon, mask = self.G.forward(motion, traj, mask=mask)
        return recon, mask
    
    def discriminate(self, x):
        short, long = self.D.forward(x)
        return short, long
...
class ContextualGenerator(nn.Module):
    ...
    def get_noise_weight(self, length):
        t0 = torch.arange(0, length,  1, dtype=torch.float32) - (self.config.context_frames-1)
        t1 = torch.arange(length, 0, -1, dtype=torch.float32) - 1
        t  = torch.clip(torch.min(t0, t1) / self.period, 0, 1)
        return t.view(1, length, 1)
    ...
    def forward(self, motion, traj, mask=None):
        ...
        """ 7. Output """
        z = torch.randn_like(x) * self.get_noise_weight(T).to(x.device)
        x = self.decoder(x + z)
